...
size_t readline(FILE** file, char** dest){
	...
	if (buffer == NULL) {
		fprintf (stderr, "Failed to allocate memory for temporary buffer in readline.\n");
        return NULL;
    }
	int c;
	buffer[len] = '\0';
	while ((c = fgetc(*file)) != EOF){
		if (len + 1 >= size){
			...
			if (tmp_buffer == NULL) {
				fprintf (stderr, "Failed to reallocate memory for temporary buffer in readline.\n");
				free(buffer);
				return NULL;
			}
			buffer = tmp_buffer;
	...
	*dest = malloc(len+1);
	if (*dest == NULL){
		fprintf(stderr, "Failed to allocate memory for line in readline.\n");
	}
	memcpy(*dest, buffer, len+1);
	free(buffer);
	return len+1;
}
...
unsigned char* hmac_sha256(const char* key, const size_t key_s, const char* msg, const size_t msg_s) {
	...
	//Set in a small localized scope for some reason?
    {
        uint64_t ipad = 0;
